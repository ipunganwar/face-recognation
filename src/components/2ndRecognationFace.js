...
export default class DoubleRecognationFace extends Component {
	capitalize(name) {
		...
		return (
			<div className="container">
				{this.props.customers.map((item, i) => (
					<div className="container-double" key={i}>
						<img
							className="images-double"
							src={`data:image/png;base64,${item.thumbnail}`}
							alt="2nd picture"
						/>
						<p className="text-name-double">
							{spesifyGender(item.additional_fields)}&nbsp;<span className="span-text-name-double">{this.capitalize(item.name)}</span>
						</p>
					</div>
				))}
